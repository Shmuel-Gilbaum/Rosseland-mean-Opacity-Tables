...
The seventh is `f05.g93`, at Grevesse & Noels 1993 ratios, reached with
``alpha=None``. It was this module's only set before the alpha sets were added.
Composition interpolation never crosses between the two, since they are
different metal mixtures rather than different points on one axis.
...
def _key(alpha):
    ...
    ------
    CoverageError
        If `alpha` is outside the tabulated range, or between two tabulated
        values. Nothing here interpolates in alpha.
    """
...
def set_name(alpha=DEFAULT_ALPHA):
...
def axes(alpha=DEFAULT_ALPHA):
    """The tabulated ``(log_T, log_R)`` axes, ascending in both.

    Parameters
    ----------
    alpha : float or None, optional
        Alpha enhancement, one of the values `alphas` lists, or None for the
        Grevesse & Noels 1993 set. Every set shares these axes.
...
def table_at(X, Z, z_floor=1e-5, alpha=DEFAULT_ALPHA):
    ...
    ndarray of shape (85, 19)
        ...
        density parameter.
    """
    Xs, Zs, log_T, log_R, K = _load(alpha)
...
def grid(log_T, log_R, X, Z, alpha=DEFAULT_ALPHA):
    ...
    """
    own_T, own_R = axes(alpha)
    block = table_at(X, Z, alpha=alpha)
